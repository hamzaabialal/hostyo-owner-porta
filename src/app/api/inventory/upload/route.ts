...
export async function POST(req: Request) {
  try {
    ...
    const blob = await put(`inventory/${filename}`, file, {
      access: "public",
      token: process.env.BLOB_READ_WRITE_TOKEN,
    });
    return NextResponse.json({ ok: true, url: blob.url, name: file.name, type: file.type, size: file.size });
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : String(error);
    console.error("Inventory upload error:", msg);
