      ...
      <div className="flex-1 flex items-center justify-center p-4 -mt-10">
        <div className="w-full max-w-[420px]">
          <div className="bg-white rounded-[28px] p-9 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15),0_8px_20px_-8px_rgba(128,2,14,0.08)]">
            {step === "form" ? (
              <>
                <h1 className="text-[22px] font-bold text-[#111] text-center mb-1">Create your account</h1>
                <p className="text-[13px] text-[#999] text-center mb-7">Get started with your owner portal</p>

                {error && (
                  <div className="mb-5 p-3 bg-[#F6EDED] border border-[#E8D8D8] rounded-xl text-[12px] text-[#7A5252] font-medium text-center">{error}</div>
                )}

                {/* Google Sign Up */}
                <button onClick={handleGoogleSignup}
                  className="w-full h-[46px] flex items-center justify-center gap-2.5 border border-[#e2e2e2] rounded-xl text-[13px] font-medium text-[#333] hover:bg-[#fafafa] hover:border-[#ccc] transition-all mb-5">
                ...
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <span className="text-[11px] text-[#ccc] font-medium">or</span>
                  <div className="flex-1 h-px bg-[#eee]" />
                </div>
                ...
                <form onSubmit={handleSendCode} className="space-y-4">
                  ...
                  <button type="submit" disabled={loading}
                    ...
                    <span className="relative">{loading ? "Sending verification..." : "Continue"}</span>
                  </button>
                </form>
              </>
            ) : (
              <>
                <div className="text-center mb-6">
                  ...
                  <h1 className="text-[22px] font-bold text-[#111] mb-1">Check your email</h1>
                  <p className="text-[13px] text-[#999]">We sent a 6-digit code to <strong className="text-[#111]">{email}</strong></p>
                </div>
                ...
                <form onSubmit={handleVerifyAndRegister} className="space-y-4">
                  ...
                  <button type="submit" disabled={loading || verifyCode.length !== 6}
                    ...
                    style={{ background: "linear-gradient(180deg, #A41826 0%, #80020E 55%, #5A0108 100%)" }}>
                    <span className="absolute inset-0 rounded-xl bg-gradient-to-b from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
                    <span className="relative">{loading ? "Verifying..." : "Verify & Create Account"}</span>
                  </button>
                </form>
          ...
          <p className="text-[13px] text-[#999] text-center mt-6">
            ...
            <a href="/login" className="text-[#111] font-semibold hover:underline">Sign in</a>
          </p>
        </div>
      </div>
    </div>
